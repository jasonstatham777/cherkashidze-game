...
function hitObstacle(player, obstacle) { if (player.isShielded) { obstacle.destroy(); player.isShielded = false; shieldEffect.setVisible(false); if (activePowerupTimers['Щит']) activePowerupTimers['Щит'].remove(); activePowerupTimers['Щит'] = null; return; } if (isGameOver) return; this.sound.play('hit_sound'); if (music) music.stop(); isGameOver = true; this.physics.pause(); player.setTint(0xff0000); activePowerupTimers = {}; totalBananas += Math.floor(score / 10); saveProgress(); const scoreResultText = gameOverContainer.getByName('scoreResultText'); scoreResultText.setText(`Собрано: ${Math.floor(score / 10)} черкашей\nВсего черкашей: ${totalBananas}`); gameOverContainer.setVisible(true); }
function saveProgress() { const progress = { totalBananas, shieldLevel, magnetLevel, scoreX2Level, isMuted }; localStorage.setItem('bananaDashProgress', JSON.stringify(progress)); }
