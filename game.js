...
function hitObstacle(player, obstacle) {
    if (player.isShielded) {
        ...
        if (activePowerupTimers['Щит']) activePowerupTimers['Щит'].remove();
        activePowerupTimers['Щит'] = null;
        return;
    }
    if (isGameOver) return;

    if (score > 0 && window.Telegram && window.Telegram.WebApp) {
        window.Telegram.WebApp.setGameScore(score, (error, result) => {
            if (error) {
    ...
    activePowerupTimers = {};
    totalBananas += Math.floor(score / 10);
    saveProgress();
    const scoreResultText = gameOverContainer.getByName('scoreResultText');
    scoreResultText.setText(`Собрано: ${Math.floor(score / 10)} черкашей\nВсего черкашей: ${totalBananas}`);
    gameOverContainer.setVisible(true);
}

function saveProgress() {
    const progress = { totalBananas, shieldLevel, magnetLevel, scoreX2Level, isMuted };
    localStorage.setItem('bananaDashProgress', JSON.stringify(progress));
}

function loadProgress() {
    const progress = JSON.parse(localStorage.getItem('bananaDashProgress'));
    if (progress) {
        totalBananas = progress.totalBananas || 0;
        shieldLevel = progress.shieldLevel || 0;
        magnetLevel = progress.magnetLevel || 0;
        scoreX2Level = progress.scoreX2Level || 0;
        isMuted = progress.isMuted || false;
    }
    shieldDuration = BASE_SHIELD_DURATION + (shieldLevel * 1500);
    magnetDuration = BASE_MAGNET_DURATION + (magnetLevel * 1500);
    scoreX2Duration = BASE_SCOREX2_DURATION + (scoreX2Level * 1500);
}

function buildGameOverMenu() {
    gameOverContainer = this.add.container(config.width / 2, config.height / 2).setVisible(false).setDepth(5);
    const bg = this.add.graphics().fillStyle(0x000000, 0.7).fillRect(-150, -150, 300, 300).lineStyle(2, 0xffffff, 1).strokeRect(-150, -150, 300, 300);
    const title = this.add.text(0, -120, 'ИГРА ОКОНЧЕНА', { fontSize: '28px', fill: '#ff4444' }).setOrigin(0.5);
    const scoreResultText = this.add.text(0, -50, '', { fontSize: '18px', fill: '#ffffff', align: 'center' }).setOrigin(0.5).setName('scoreResultText');
    const restartBtn = this.add.text(0, 30, 'Перезапуск', { fontSize: '24px', fill: '#00ff00' }).setOrigin(0.5).setInteractive();
    const shopBtn = this.add.text(0, 90, 'Магазин', { fontSize: '24px', fill: '#ffff00' }).setOrigin(0.5).setInteractive();
    restartBtn.on('pointerdown', () => this.scene.start('GameScene'));
    shopBtn.on('pointerdown', () => {
        gameOverContainer.setVisible(false);
        updateShopText.call(this);
        shopContainer.setVisible(true);
    });
    [restartBtn, shopBtn].forEach(btn => {
        btn.on('pointerover', () => btn.setScale(1.1));
        btn.on('pointerout', () => btn.setScale(1));
    });
    gameOverContainer.add([bg, title, scoreResultText, restartBtn, shopBtn]);
}

function buildShopMenu() {
    shopContainer = this.add.container(config.width / 2, config.height / 2).setVisible(false).setDepth(5);
    const bg = this.add.graphics().fillStyle(0x000000, 0.85).fillRect(-180, -250, 360, 500).lineStyle(2, 0xffffff, 1).strokeRect(-180, -250, 360, 500);
    const title = this.add.text(0, -220, 'МАГАЗИН', { fontSize: '28px', fill: '#ffff00' }).setOrigin(0.5);
    const bananasText = this.add.text(0, -180, '', { fontSize: '20px', fill: '#ffffff' }).setOrigin(0.5).setName('bananasText');
    const shieldInfo = this.add.text(-160, -120, '', { fontSize: '16px', fill: '#87CEEB', align: 'left' }).setOrigin(0, 0).setName('shieldInfo');
    const shieldBtn = this.add.text(120, -110, 'Купить', { fontSize: '18px', fill: '#00ff00', backgroundColor: '#333', padding: { x: 10, y: 5 } }).setOrigin(0.5).setInteractive();
    const magnetInfo = this.add.text(-160, -40, '', { fontSize: '16px', fill: '#FFB6C1', align: 'left' }).setOrigin(0, 0).setName('magnetInfo');
    const magnetBtn = this.add.text(120, -30, 'Купить', { fontSize: '18px', fill: '#00ff00', backgroundColor: '#333', padding: { x: 10, y: 5 } }).setOrigin(0.5).setInteractive();
    const scoreX2Info = this.add.text(-160, 40, '', { fontSize: '16px', fill: '#FFD700', align: 'left' }).setOrigin(0, 0).setName('scoreX2Info');
    const scoreX2Btn = this.add.text(120, 50, 'Купить', { fontSize: '18px', fill: '#00ff00', backgroundColor: '#333', padding: { x: 10, y: 5 } }).setOrigin(0.5).setInteractive();
    const backBtn = this.add.text(0, 200, 'Назад', { fontSize: '24px', fill: '#ff4444' }).setOrigin(0.5).setInteractive();
    shieldBtn.on('pointerdown', () => buyUpgrade.call(this, 'shield'));
    magnetBtn.on('pointerdown', () => buyUpgrade.call(this, 'magnet'));
    scoreX2Btn.on('pointerdown', () => buyUpgrade.call(this, 'scoreX2'));
    backBtn.on('pointerdown', () => {
        shopContainer.setVisible(false);
        gameOverContainer.setVisible(true);
    });
    [shieldBtn, magnetBtn, scoreX2Btn, backBtn].forEach(btn => {
        btn.on('pointerover', () => btn.setScale(1.1));
        btn.on('pointerout', () => btn.setScale(1));
    });
    shopContainer.add([bg, title, bananasText, shieldInfo, shieldBtn, magnetInfo, magnetBtn, scoreX2Info, scoreX2Btn, backBtn]);
}

function buyUpgrade(type) {
    const costs = {
        shield: 100 + (shieldLevel * 150),
        magnet: 150 + (magnetLevel * 200),
        scoreX2: 200 + (scoreX2Level * 250)
    };
    const cost = costs[type];
    if (totalBananas >= cost) {
        totalBananas -= cost;
        if (type === 'shield') shieldLevel++;
        else if (type === 'magnet') magnetLevel++;
        else if (type === 'scoreX2') scoreX2Level++;
        loadProgress();
        saveProgress();
        updateShopText.call(this);
    } else {
        this.sound.play('no_money_sound');
    }
}

function updateShopText() {
    shopContainer.getByName('bananasText').setText(`Ваши черкаши: ${totalBananas}`);
    const shieldCost = 100 + (shieldLevel * 150);
    shopContainer.getByName('shieldInfo').setText(`Щит (Ур. ${shieldLevel})\nДлительность: ${(shieldDuration / 1000).toFixed(1)} сек.\nСтоимость: ${shieldCost}`);
    const magnetCost = 150 + (magnetLevel * 200);
    shopContainer.getByName('magnetInfo').setText(`Магнит (Ур. ${magnetLevel})\nДлительность: ${(magnetDuration / 1000).toFixed(1)} сек.\nСтоимость: ${magnetCost}`);
    const scoreX2Cost = 200 + (scoreX2Level * 250);
    shopContainer.getByName('scoreX2Info').setText(`Очки x2 (Ур. ${scoreX2Level})\nДлительность: ${(scoreX2Duration / 1000).toFixed(1)} сек.\nСтоимость: ${scoreX2Cost}`);
}
